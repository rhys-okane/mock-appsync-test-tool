...
if (!process.env.APPSYNC_EVENTS_API_URL ||
    !process.env.APPSYNC_EVENTS_REGION ||
    !process.env.APPSYNC_EVENTS_API_KEY) {
  throw new Error("Missing required environment variables for AppSync events API configuration.");
}
...
export const handler = async (event: AppSyncResolverEvent<unknown>): Promise<unknown> => {
  ...
  return await new Promise<unknown>(async (resolve, reject) => {
    ...
    events.post('/default/appsync-lambda-invokes', <any>{
